  ...
  const recommendations = [
    {
      ...
      bgGlow: 'bg-primary/5 hover:bg-primary/10',
    ...
    },
    {
      ...
      bgGlow: 'bg-tertiary/5 hover:bg-tertiary/10',
      borderColor: 'hover:border-tertiary/30'
    }
  ];

  return (
    <section className="w-full lg:w-[22%] p-4 border-l border-outline-variant flex flex-col gap-6 overflow-y-auto z-10 relative">
      
      {/* Title */}
      <div>
        <h3 className="font-mono text-[10px] font-bold text-on-surface-variant/60 mb-4 tracking-widest uppercase">
          AI Recommendations
        </h3>
        <div className="flex flex-col gap-4">
          {recommendations.map((rec, idx) => {
            ...
            return (
              <div 
                key={idx}
                className={`glass-card rounded-2xl p-4 relative overflow-hidden group ${rec.bgGlow} ${rec.borderColor} border border-outline-variant transition-all duration-300`}
              >
                ...
                <p className="font-sans text-xs font-semibold text-on-surface leading-tight mb-3">
                  {rec.description}
                </p>
      ...
      <div className="glass-card rounded-2xl overflow-hidden border border-outline-variant shadow-md shadow-black/20">
        ...
        <div className="p-3 bg-surface-low">
          <p className="text-[9px] font-mono text-primary uppercase tracking-wider font-extrabold">
      ...
      <div className="mt-auto pt-2">
        <div className="p-4 bg-tertiary-container/10 border border-tertiary-container/20 rounded-2xl flex flex-col gap-2">
          <p className="text-xs text-tertiary font-bold flex items-center gap-1.5 leading-none">
            <Lightbulb className="w-4 h-4 animate-bounce shrink-0" />
            Pro Tip
          </p>
          <p className="text-[10px] text-on-surface-variant leading-relaxed font-sans font-medium">
